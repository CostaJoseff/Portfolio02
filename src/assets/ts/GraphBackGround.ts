import { CompartilhamentoService } from '../../app/services/compartilhamento.service';
...
export class GraphBackGround {
  ...
  private coresCompartilhadas: CompartilhamentoService;
  ...
  constructor(altura: any, largura: any, quantidadeDePontos: number, velocidadeDosPontos: number, coresCompartilhadas: CompartilhamentoService, animado: boolean, tamanhoDosPontos: number = 3, espessuraDasLinhas: number = 1) {
    ...
    if (this.largura <= 720 || this.altura <= 720) {
      this.quantidadeDePontos = 100;
  ...
  private desenharPontos() {
    this.pontos.forEach((ponto) => {
      this.ctx.fillStyle = `rgba(${this.coresCompartilhadas.corPrincipal[0]}, ${this.coresCompartilhadas.corPrincipal[1]}, ${this.coresCompartilhadas.corPrincipal[2]})`;
      this.ctx.beginPath();
  ...
  private linkarDesenharLinks(ponto1: any, ponto2: any, distancia: number) {
    ...
    this.ctx.strokeStyle = `rgba(${this.coresCompartilhadas.corSecundaria[0]}, ${this.coresCompartilhadas.corSecundaria[1]}, ${this.coresCompartilhadas.corSecundaria[2]}, ${this.calcularNivelDeTransparencia(distancia)})`;
    this.ctx.stroke();
    this.ctx.closePath();
